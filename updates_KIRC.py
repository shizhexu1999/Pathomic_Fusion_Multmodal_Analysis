import torch
import torch_geometric
import os
from tqdm import tqdm


def reformat_graph_data(data):
    """
    ensure that the object structure is correct
    """
    # use `data.py` module within the `data` subpackage
    return torch_geometric.data.data.Data.from_dict(data.__dict__)


for file in tqdm(os.listdir("data/TCGA_KIRC/KIRC_st_cpc_old/pt_bi")):
    old_data = torch.load(
        os.path.join("data/TCGA_KIRC/KIRC_st_cpc_old/pt_bi", file),
    )
    torch.save(
        reformat_graph_data(old_data),
        os.path.join("data/TCGA_KIRC/KIRC_st_cpc/pt_bi", file),
    )
