...
from functions import CIndex_lifeline, cox_log_rank, changeHistomolecularSubtype
...
# Glioma
def getGradTestPats_GBMLGG(ckpt_name='./checkpoints/TCGA_GBMLGG/grad_15_paper/', model='pathgraphomic_fusion', split='test', use_rnaseq=False, agg_type='mean'):
    pats = {}
    ...
    data_cv_path = 'data/TCGA_GBMLGG/splits_original/gbmlgg15cv_%s_%d_%d_%d%s.pkl' % (roi_dir, ignore_missing_moltype, ignore_missing_histype, use_vgg_features, use_rnaseq, )
    print(data_cv_path)
    for k in range(1,16):
...
def getPredAggGrad_GBMLGG(ckpt_name='checkpoints/TCGA_GBMLGG/grad_15_paper/', model='pathgraphomic_fusion', split='test', use_rnaseq=False, 
    ...
    use_rnaseq = '_rnaseq' if use_rnaseq else ''
    if 'path' == model:
        data_cv_path = './data/TCGA_GBMLGG/splits_original/gbmlgg15cv_CNN_all_st_patches_512_0_1_1.pkl'
    else:
        data_cv_path = './data/TCGA_GBMLGG/splits_original/gbmlgg15cv_%s_%d_%d_%d%s.pkl' % (roi_dir, ignore_missing_moltype, ignore_missing_histype, use_vgg_features, use_rnaseq, )
...
def calcGradMetrics(y_label_all, y_pred_all, avg='micro'):
    ...
    for i in range(15):
        ...
        rocauc_all.append(roc_auc_score(y_label, y_pred, average='micro'))
...
def getSurvTestPats_GBMLGG(ckpt_name='checkpoints/TCGA_GBMLGG/surv_15_paper/', model='pathgraphomic_fusion', split='test', use_rnaseq=True, agg_type='Hazard_mean'):
    pats = {}
    ...
    for k in range(1,16):
        pred = pickle.load(open(ckpt_name+'/%s/%s_%d%spred_%s.pkl' % (model, model, k, use_patch, split), 'rb'))
        print(len(pred))    
        surv_all = pd.DataFrame(np.stack(np.delete(np.array(pred, dtype=object), 3))).T
...
def getPValAggSurv_GBMLGG_Multi(ckpt_name='./checkpoints/TCGA_GBMLGG/surv_15_paper/', model='pathgraphomic_fusion', percentile=[33,66]):
    data = getDataAggSurv_GBMLGG(ckpt_name=ckpt_name, model=model)
    p = np.percentile(data['Hazard'], percentile)
...
def getPredAggSurv_GBMLGG(ckpt_name='./checkpoints/TCGA_GBMLGG/surv_15_paper/', model='pathgraphomic_fusion', 
    ...
    if 'cox' in model:
        ...
        ignore_missing_moltype = 1 if 'omic' in model else 0
        ignore_missing_histype = 1 if 'grad' in ckpt_name else 0
        use_patch, roi_dir, use_vgg_features = ('_patch_', 'all_st_patches_512', 1) if (('path' in model) or ('graph' in model)) or ('multilinear_fusion' in model) else ('_', 'all_st', 0)
        use_rnaseq = '_rnaseq' if ('path' != model and 'pathpath' not in model and 'graph' != model and 'graphgraph' not in model) else ''
        if 'path' == model:
            data_cv_path = './data/TCGA_GBMLGG/splits_original/gbmlgg15cv_all_st_patches_512_0_0_1_new.pkl'
            data_cv = pickle.load(open(data_cv_path, 'rb'))
        elif 'multilinear_fusion' == model:
            data_cv_path = './data/TCGA_GBMLGG/splits_original/gbmlgg15cv_all_st_patches_512_1_0_1_rnaseq_conch.pkl'
            data_cv = pickle.load(open(data_cv_path, 'rb'))
        else:
            data_cv_path = './data/TCGA_GBMLGG/splits_original/gbmlgg15cv_%s_%d_%d_%d%s.pkl' % (roi_dir, ignore_missing_moltype, ignore_missing_histype, use_vgg_features, use_rnaseq, )
            data_cv = pickle.load(open(data_cv_path, 'rb'))
...
def getDataAggSurv_GBMLGG(ckpt_name='./checkpoints/TCGA_GBMLGG/surv_15_paper/', model='pathgraphomic_fusion', 
    ...
    if 'cox' in model:
        ...
        ignore_missing_moltype = 1 if ('omic' in model) or ('multilinear_fusion' in model) else 0
        ignore_missing_histype = 1 if 'grad' in ckpt_name else 0
        use_patch, roi_dir, use_vgg_features = ('_patch_', 'all_st_patches_512', 1) if (('path' in model) or ('graph' in model)) or ('multilinear_fusion' in model) else ('_', 'all_st', 0)
        use_rnaseq = '_rnaseq' if ('path' != model and 'pathpath' not in model and 'graph' != model and 'graphgraph' not in model) else ''
        if 'path' == model:
...
def makeHazardSwarmPlot(ckpt_name='./checkpoints/surv_15_rnaseq/', model='path', split='test', zscore=True, agg_type='Hazard_mean'):
    ...
    #     logging.info(f"Duplicates in index found: {num_duplicates} duplicates")
...
### KIRC
def getDataAggSurv_KIRC(ckpt_name='./checkpoints/TCGA_KIRC/surv_15_paper/', model='pathgraphomic_fusion', 
    ...
    if 'cox' in model:
        ...
        data_cv = pickle.load(open('./data/TCGA_KIRC/splits_original/KIRC_st_1.pkl', 'rb'))
...
def getHazardHistogramPlot_KIRC(ckpt_name='./checkpoints/TCGA_KIRC/surv_15_paper/', model='pathgraphomic_fusion', 
...
def makeKaplanMeierPlot_KIRC_Binary(ckpt_name='./checkpoints/TCGA_KIRC/surv_15_paper/', model='omic', split='test', zscore=False, agg_type='Hazard_mean', percentile=[50]):
...
def makeKaplanMeierPlot_KIRC_Multi(ckpt_name='./checkpoints/TCGA_KIRC/surv_15_paper/', model='omic', split='test', zscore=False, agg_type='Hazard_mean', percentile=[25, 50, 75]):
    def hazard2KMCurve(data, percentile):
        p = np.percentile(data['Hazard'], percentile)
        print(p)
        # return None
    ...
    fig = hazard2KMCurve(data, percentile=percentile)
    # return None
    fig.savefig(ckpt_name+'/%s_%s_KM_%s.png' % ('./checkpoints/TCGA_KIRC/surv_15_paper/'.split('/')[2]+"_Multi", model, 'all_lines'), bbox_inches='tight')
    plt.close()


def makeKaplanMeierPlot_Baseline_grade(ckpt_name='./checkpoints/TCGA_GBMLGG/surv_15_rnaseq/', model='Grade'):
    def hazard2KMCurve(data, model):
        fig = plt.figure(figsize=(10, 10), dpi=600)
        ax = plt.subplot()
        censor_style = {'ms': 20, 'marker': '+'}
    ...
    fig = hazard2KMCurve(data, model)
    fig.savefig(ckpt_name+'/grade_KM.png', bbox_inches='tight')


def makeKaplanMeierPlot_Baseline_molecular(ckpt_name='./checkpoints/TCGA_GBMLGG/surv_15_rnaseq/', model='Grade'):
    def hazard2KMCurve(data, model):
        fig = plt.figure(figsize=(10, 10), dpi=600)
        ax = plt.subplot()
        censor_style = {'ms': 20, 'marker': '+'}
    ...
    fig = hazard2KMCurve(data, model)
    fig.savefig(ckpt_name+'/molecular_KM.png', bbox_inches='tight')