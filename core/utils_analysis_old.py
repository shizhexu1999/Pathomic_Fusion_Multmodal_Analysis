...
def natural_sort(l): 
    """
    It sorts a list of strings with human expectations rather than Python's build-in sort.
    """
    convert = lambda text: int(text) if text.isdigit() else text.lower() 
    # split the input string at point where a numeric substring occurs.
    alphanum_key = lambda key: [ convert(c) for c in re.split('([0-9]+)', key) ] 
    return sorted(l, key = alphanum_key)

# example usage
# file_list = ['file1.txt', 'file10.txt', 'file2.txt']
...
def getGradTestPats_GBMLGG(ckpt_name='./checkpoints/TCGA_GBMLGG/grad_15/', model='pathgraphomic_fusion', split='test', use_rnaseq=False, agg_type='mean'):
    """
    It returns the specific TCAG IDs for the grade classification results in the test split.
    """
    pats = {}
    ...
    for k in range(1,16):
        ...
        assert np.all(data_cv_split_k[split]['g'] == pred[4])
        all_dataset = data_cv['data_pd'].drop('TCGA ID', axis=1)
        all_dataset_regstrd = all_dataset.loc[data_cv_split_k[split]['x_patname']]
...
def getPredAggGrad_GBMLGG(ckpt_name='checkpoints/TCGA_GBMLGG/grad_15/', model='pathgraphomic_fusion', split='test', use_rnaseq=False, 
    ...
    """
    It is used to check if the predictions are correctly registered with the TCGA IDs,
    and use the agg_type = 'max' to get the final predictions.
    It returns the predictions and ground truth for the selected TCGA IDs/patient names.
    """
    y_label, y_pred = [], []
    ...
    for k in range(1,16):
        # load prediction pickle file
        pred = pickle.load(open(ckpt_name+'/%s/%s_%d%spred_%s.pkl' % (model, model, k, use_patch, split), 'rb'))    
        grad_pred = pred[3].T
        grad_pred = pd.DataFrame(np.stack(grad_pred)).T
        # prepare three same columns for the `calcGradMetrics` function later
        grad_pred.columns = ['score_0', 'score_1', 'score_2']
        data_cv = pickle.load(open(data_cv_path, 'rb'))
        data_cv_splits = data_cv['cv_splits']
        data_cv_split_k = data_cv_splits[k]
        # check if the grade information is correctly registered with the TCGA IDs
        assert np.all(data_cv_split_k[split]['g'] == pred[4])
        all_dataset = data_cv['data_pd'].drop('TCGA ID', axis=1)
        # gives a subset of "all_datasets" (metadata) that is registered with "pred" (predictions)
        all_dataset_regstrd = all_dataset.loc[data_cv_split_k[split]['x_patname']]
        assert np.all(np.array(all_dataset_regstrd['Grade']) == pred[4])
        ...
        # it calculates the aggregation of the predictions for each patient (using max)
        grad_pred = grad_pred.groupby('TCGA ID').agg({'score_0': [fun], 'score_1': [fun], 'score_2': [fun]})
        test_pat = test_pats[k]
        # it extracts the corresponding grades of predictions for the patitent IDs (test_pat)
        grad_pred = grad_pred.loc[test_pat]
        # it extracts the corresponding grades of ground truth for the patient IDs (test_pat)
        # the IDs come from the test splits
        grad_gt = np.array(all_dataset.loc[test_pat]['Grade'])
        grad_pred = np.array(grad_pred)
        # it transforms multi-class labels to binary labels
        enc = LabelBinarizer()
        # fit on the ground truth data
        enc.fit(grad_gt)
        # convert the categorical labels in grad_gt into a binary/one-hot encoded format
...
def trainCox_GBMLGG(dataroot = './data/TCGA_GBMLGG/', ckpt_name='./checkpoints/TCGA_GBMLGG/surv_15_rnaseq/', model='cox_omic', use_rnaseq=False, normalize=False, penalizer=0):
    ...
    for k in pnas_splits.columns:
        ...
        # we take cox_agegender, cox_moltype, cox_grade and cox_molgrade
        ...
        # concordance_index(survtime_all, -hazards, labels), censored is used as labels
...
def getSurvTestPats_GBMLGG(ckpt_name='./checkpoints/TCGA_GBMLGG/surv_15_rnaseq/', model='pathgraphomic_fusion', split='test', use_rnaseq=True, agg_type='Hazard_mean'):
    ...
    for k in range(1,16):
        pred = pickle.load(open(ckpt_name+'/%s/%s_%d%spred_%s.pkl' % (model, model, k, use_patch, split), 'rb'))
        # add `dtype = object` to `np.array(pred)` to solve the issue of numpy version 
...
def getPValAggSurv_GBMLGG_Multi(ckpt_name='./checkpoints/TCGA_GBMLGG/surv_15_rnaseq/', model='pathgraphomic_fusion', percentile=[33,66]):
    data = getDataAggSurv_GBMLGG(ckpt_name=ckpt_name, model=model)
    # two thresholds for 33 and 66
    p = np.percentile(data['Hazard'], percentile)
    if p[0] == p[1]: p[0] = 2.99997
    # the new data column is inserted at the very beginning of the dataframe
    # there are three grades: 0, 1, 2 because we have two thresholds
    data.insert(0, 'grade_pred', [hazard2grade(hazard, p) for hazard in data['Hazard']])
...
def getDataAggSurv_GBMLGG(ckpt_name='./checkpoints/TCGA_GBMLGG/surv_15_rnaseq/', model='pathgraphomic_fusion', 
    ...
    if 'cox' in model:
        ...
        for k in range(1,16):
            ...
            all_dataset = data_cv['data_pd'].drop('TCGA ID', axis=1)
            # subset of "all_datasets" (metadata) that is registered with "pred" (predictions)
            all_dataset_regstrd = all_dataset.loc[data_cv_split_k[split]['x_patname']]
            assert np.all(np.array(all_dataset_regstrd['Survival months']) == pred[1])
            assert np.all(np.array(all_dataset_regstrd['censored']) == pred[2])
            assert np.all(np.array(all_dataset_regstrd['Grade']) == pred[4])
            ...
            hazard_agg = all_dataset_regstrd.groupby('TCGA ID').agg({'Hazard': ['mean', 'median', max]})
            # flatten the multi-level column index into a 1D array
            # new column names include 'Hazard_mean', 'Hazard_median', and 'Hazard_max'
            hazard_agg.columns = ["_".join(x) for x in hazard_agg.columns.ravel()]
            hazard_agg = hazard_agg[[agg_type]]
            hazard_agg.columns = ['Hazard']
            all_dataset_hazard = hazard_agg.join(all_dataset, how='inner')
...
def makeHazardSwarmPlot(ckpt_name='./checkpoints/surv_15_rnaseq/', model='path', split='test', zscore=True, agg_type='Hazard_mean'):
    ...
    # exclude the missing labels which represents for 1
    data = data[data['Grade'] != -1]
    data = data[data['Histomolecular subtype'] != -1]
    data['Grade'] = data['Grade'].astype(int).astype(str)
    # change grade to the notations set in the original paper
    ...
    # change histomolecular subtype to the notations set in the original paper
    ...
    # specify custom colors for different levels of the categorical variable specified by the hue parameter
    ...
    # add an additional folder path to save the figure
...
def makeKaplanMeierPlot(ckpt_name='./checkpoints/TCGA_GBMLGG/surv_15_rnaseq/', model='omic', split='test', zscore=False, agg_type='Hazard_mean', plot_gt=True):
    def hazard2KMCurve(data, subtype, plot_gt=True):
        ...
        if p[0] == p[1]: p[0] = 2.99997
        # it calculates the percentile threshold
        data.insert(0, 'grade_pred', [hazard2grade(hazard, p) for hazard in data['Hazard']])
...
def CI_pm(data, confidence=0.95):
    """
    Function calculates the confidence interval of the data
    """
    # convert into a numpy array first
    a = 1.0 * np.array(data)
    n = len(a)
    # mean and standard derror
    m, se = np.mean(a), scipy.stats.sem(a)
    # calculate the percentile to use with the t-distribution
    h = se * scipy.stats.t.ppf((1 + confidence) / 2., n-1)
    return str("{0:.4f} ± ".format(m) + "{0:.3f}".format(h))
