...
def getGradTestPats_GBMLGG(ckpt_name='./checkpoints/TCGA_GBMLGG/grad_15/', model='pathgraphomic_fusion', split='test', use_rnaseq=False, agg_type='mean'):
    ...
    data_cv_path = 'data/TCGA_GBMLGG/splits_conch/gbmlgg15cv_%s_%d_%d_%d%s.pkl' % (roi_dir, ignore_missing_moltype, ignore_missing_histype, use_vgg_features, use_rnaseq, )
    print(data_cv_path)
    for k in range(1,16):
...
def calcGradMetrics(y_label_all, y_pred_all, avg='micro'):
    ...
    for i in range(15):
        ...
        rocauc_all.append(roc_auc_score(y_label, y_pred, avg=avg))
...
def getSurvTestPats_GBMLGG(ckpt_name='checkpoints/TCGA_GBMLGG/surv_15_conch/', model='pathgraphomic_fusion', split='test', use_rnaseq=True, agg_type='Hazard_mean'):
    pats = {}
...
def getPValAggSurv_GBMLGG_Multi(ckpt_name='./checkpoints/TCGA_GBMLGG/surv_15_conch/', model='pathgraphomic_fusion', percentile=[33,66]):
    data = getDataAggSurv_GBMLGG(ckpt_name=ckpt_name, model=model)
    p = np.percentile(data['Hazard'], percentile)
...
def getPredAggSurv_GBMLGG(ckpt_name='./checkpoints/TCGA_GBMLGG/surv_15_conch/', model='pathgraphomic_fusion', 
    ...
    if 'cox' in model:
        ...
        if 'path' == model:
            data_cv_path = './data/TCGA_GBMLGG/splits_conch/gbmlgg15cv_CNN_all_st_patches_512_0_0_1.pkl'
            data_cv = pickle.load(open(data_cv_path, 'rb'))
        else:
            data_cv_path = './data/TCGA_GBMLGG/splits_conch/gbmlgg15cv_%s_%d_%d_%d%s.pkl' % (roi_dir, ignore_missing_moltype, ignore_missing_histype, use_vgg_features, use_rnaseq, )
            data_cv = pickle.load(open(data_cv_path, 'rb'))
...
def getDataAggSurv_GBMLGG(ckpt_name='./checkpoints/TCGA_GBMLGG/surv_15_conch/', model='pathgraphomic_fusion', 
