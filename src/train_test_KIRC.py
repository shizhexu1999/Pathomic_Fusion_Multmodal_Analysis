"""!@file train_test_KIRC.py
@brief This file contains the training and testing functions for the KIRC dataset.
@author Shizhe Xu
@date 28 June 2024
"""
...
def train(opt, data, device, k):
    ...
    # )
    # we may can discard PathgraphomicDatasetLoader here
    custom_data_loader = (
    ...
    for epoch in tqdm(range(opt.epoch_count, opt.niter + opt.niter_decay + 1)):
        ...
        for batch_idx, (x_path, x_grph, x_omic, censor, survtime, grade) in enumerate(
            ...
            # pred is predicted hazard score
            ...
            pickle.dump(
                ...
                open(
                    os.path.join(
                        ...
                        opt.exp_name,
                        opt.model_name,
                        "%s_%d_%d_pred_test.pkl"
                        % (opt.model_name, k, epoch),
                    ),
                    "wb",
                ),
            )
...
def test(opt, model, data, split, device):
    ...
    for batch_idx, (x_path, x_grph, x_omic, censor, survtime, grade) in enumerate(
        ...
        if opt.task == "surv":
            ...
            # adjust the shape of the grade tensor to match that of grade_pred
            # check for equality between grade_pred and the ground truth grade
            # sum reprsents the total number of correct predictions in the current batch
