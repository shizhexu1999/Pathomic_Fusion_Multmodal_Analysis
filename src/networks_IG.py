"""!@file networks_IG.py
@brief This file is used to define the network architecture for IG.
@details It conducts IG visulisation with fusion_IG.py that solves the dimensionality issue.
@author Shizhe Xu
@date 28 June 2024
"""
...
class NormalizeFeaturesV2(object):
    ...
    def __call__(self, data):
        ...
        # Perform the normalization without in-place operation
        normalized_x = torch.cat((data.x[:, :12] / max_values, data.x[:, 12:]), dim=1)
        data.x = normalized_x
...
class PathgraphomicNet(nn.Module):
    ...
    def forward(self, **kwargs):
        ...
        grph_vec, _ = self.grph_net(x_grph=kwargs["x_grph"])
        omic_vec, _ = self.omic_net(x_omic=kwargs["x_omic"])
        # print(f"path_vect", path_vec.shape)
        # print(f"grph_vec", grph_vec.shape)
        # print(f"omic_vec", omic_vec.shape)
