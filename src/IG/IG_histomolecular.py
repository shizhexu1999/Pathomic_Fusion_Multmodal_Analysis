"""!@file IG_histomolecular.py
@breif This file is used to do global explanation for genomic features, SNN, via Integrated Gradients.
@details We use the Integrated Gradients to explain the genomic features, and we use the data splits specified by the histomolecular subtypes.
@author Shizhe Xu
@date 28 June 2024
"""
...
def predictions_and_gradients(inputs, target_label_index, model):
    ...
    """
    inputs = torch.tensor(inputs, requires_grad=True).to(
        device
    )  # Ensure inputs are on the MPS device
    features, predictions = model(x_omic=inputs)
    gradients = grad(
        outputs=predictions,
        inputs=inputs,
        grad_outputs=torch.ones_like(predictions).to(
            device
        ),  # Ensure grad_outputs are on the MPS device
        create_graph=True,
    )[0]
...
def integrated_gradients(
    inp, target_label_index, predictions_and_gradients, baseline=None, steps=50
):
    ...
    scaled_inputs = [
        baseline + (float(i) / steps) * (inp - baseline) for i in range(steps + 1)
    ]
    scaled_inputs = torch.stack(scaled_inputs).to(
        device
    )  # Convert to tensor and move to MPS device
    predictions, grads = predictions_and_gradients(
        scaled_inputs.cpu().numpy(), target_label_index
    )  # shapes: <steps+1>, <steps+1, inp.shape>

    # Use trapezoidal rule to approximate the integral.
    grads = torch.tensor(grads).to(
        device
    )  # Convert gradients to tensor and move to MPS device
...
def plot_shap_style_integrated_gradients(
    attributions,
    features,
    top_n=20,
    xlim=(-2, 2),
...
data_cv_path_train = "%s/splits_IG/gbmlgg15cv_%s_%d_%d_%d%s.pkl" % (
    opt.dataroot,
    roi_dir,
...
data_cv_path_test = "%s/splits_IG/gbmlgg15cv_%s_%d_%d_%d%s_%s.pkl" % (
    opt.dataroot,
    roi_dir,
    ignore_missing_moltype,
    ignore_missing_histype,
...
print("Loading train %s" % data_cv_path_train)
print("Loading test %s" % data_cv_path_test)

data_cv_train = pickle.load(open(data_cv_path_train, "rb"))
data_cv_test = pickle.load(open(data_cv_path_test, "rb"))
data_cv_splits_train = data_cv_train["cv_splits"]
data_cv_splits_test = data_cv_test["cv_splits"]

first_key_train = next(iter(data_cv_splits_train))
data_train = data_cv_splits_train[first_key_train]

first_key_test = next(iter(data_cv_splits_test))
data_test = data_cv_splits_test[first_key_test]

custom_data_loader_train = (
    PathgraphomicFastDatasetLoader(opt, data_train, split="all", mode=opt.mode)
    if opt.use_vgg_features
    else PathgraphomicDatasetLoader(opt, data_train, split="all", mode=opt.mode)
)
custom_data_loader_test = (
    PathgraphomicFastDatasetLoader(opt, data_test, split="all", mode=opt.mode)
    if opt.use_vgg_features
    else PathgraphomicDatasetLoader(opt, data_test, split="all", mode=opt.mode)
)
...
for batch_idx, (x_path, x_grph, x_omic, censor, survtime, grade) in enumerate(
    ...
    integrated_grads, prediction_trend = integrated_gradients(
        input_tensor,
        target_label_index,
        lambda inputs, label: predictions_and_gradients(inputs, label, model),
        baseline=None,
        steps=50,
    )
    print(
        "Integrated gradients shape:", integrated_grads.shape
    )  # Expected shape: (input_dim,)
...
filename = "gbmlgg_%s_shap_style_plot_%s.png" % (
    roi_dir,
    opt.histomolecular_type,
)
