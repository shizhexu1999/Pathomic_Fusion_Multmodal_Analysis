...
@details The integrated gradients method is used to generate global explanations by using CNN+SNN models.
@author Shizhe Xu
@date 28 June 2024
"""
...
def predictions_and_gradients(inputs_path, inputs_omic, target_label_index, model):
    """
    ...
    features, predictions = model(x_path=inputs_path, x_omic=inputs_omic)
    gradients = grad(
        outputs=predictions,
        inputs=inputs_omic,
...
def integrated_gradients(
    ...
    predictions, grads = predictions_and_gradients(scaled_inputs_path.cpu().numpy(), scaled_inputs_omic.cpu().numpy(), target_label_index)  # shapes: <steps+1>, <steps+1, inp.shape>
...
for batch_idx, (x_path, x_grph, x_omic, censor, survtime, grade) in enumerate(
    ...
    # x_grph = x_grph.to(device)
    # print(x_grph.shape)
    print(input_tensor_path)
    print(input_tensor_omic)
    # we define the target label index (not used in regression, but required by function signature)
    ...
    integrated_grads, prediction_trend = integrated_gradients(input_tensor_path, input_tensor_omic,target_label_index, lambda path_inputs, omic_inputs, label: predictions_and_gradients(path_inputs, omic_inputs, label, model), baseline_path=None, baseline_omic=None, steps=50)
...
plot_shap_style_integrated_gradients(
    ...
    filename="path+omic_%s.png" % opt.histomolecular_type,
)