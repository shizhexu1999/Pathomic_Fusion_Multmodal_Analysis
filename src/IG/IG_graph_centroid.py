"""!@file IG_graph_centroid.py
@breif This file is used to do local explanation for cell graphs, GCN, via Integrated Gradients.
@details The centroid of the graph is used as the node positions for the graph, and we avoid the complicated edge connecting scenario.
@author Shizhe Xu
@date 28 June 2024
"""
...
def plot_cell_graph_heatmap(graph_data, node_attributions, node_positions, save_folder="./visualisation_plots/", filename="shap_style_plot.png",):
    G = nx.Graph()
    ...
    # nx.draw(G, pos, node_color=node_colors, with_labels=True, node_size=300, cmap=plt.cm.viridis, ax=ax)
    nx.draw(G, pos, node_color=node_colors, with_labels=False, node_size=50, cmap=plt.cm.viridis, ax=ax)
...
if integrated_grads.shape[0] != graph_data.num_nodes:
    ...
    # Plot the cell graph heatmap
    plot_cell_graph_heatmap(graph_data, node_attributions, node_positions, save_folder="./visualisation_plots/", filename="IG_graph_centroid.png",)