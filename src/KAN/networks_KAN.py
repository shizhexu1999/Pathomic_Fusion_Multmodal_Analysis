"""!@file networks_KAN.py
@brief This file is used to define unimodal network with KANs.
@details We only modify the omic model in this file, and the other models are the same as the original models.
@author Shizhe Xu
@date 28 June 2024
"""
...
def define_net(opt, k):
    ...
    if opt.mode == "path":
        ...
        #     init_max=init_max,
        # )
        # net = KAN(width=[opt.input_size_omic,64, 48, 32, 32,opt.label_dim], grid=3, k=3, seed=0)
        # 64 neurons for 46 minutes for each epoch
        # 10 neurons for 8 minutes for each epoch
...
class GraphNet(torch.nn.Module):
    def __init__(
        ...
        self.pool1 = torch_geometric.nn.SAGPooling(
            nhid, ratio=pooling_ratio, 
            GNN=torch_geometric.nn.GraphConv
        )
