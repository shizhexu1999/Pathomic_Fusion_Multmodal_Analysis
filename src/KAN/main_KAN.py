"""!@file main_KAN.py
@brief This file is used to train and test unimodal network on genomic modality, but MLPs have been replaced by KANs.
@details The file uses the dataloader, options, networks_KAN and train_test_KAN files.
@author Shizhe Xu
@date 28 June 2024
"""
...
from train_test_KAN import train, test
...
for k, data in data_cv_splits.items():
    ...
    while attempts < max_attempts:
        try:
            # pred = train(opt, data, device, k)
            model, optimizer, metric_logger, loss_test, cindex_test, pvalue_test, surv_acc_test, grad_acc_test, pred_test = train(opt, data, device, k)
