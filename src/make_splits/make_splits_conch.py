"""!@file make_splits_conch.py
@brief This file is used to create the 15 fold cross validation splits that use CONCH encoder.
@author Shizhe Xu
@date 28 June 2024
"""
...
for k in pnas_splits.columns:
    ...
    if opt.use_vgg_features:
        ...
        model = define_net_conch(opt, None)
...
pickle.dump(
    data_dict,
    open(
        "%s/splits_conch/gbmlgg15cv_%s_%d_%d_%d%s.pkl"
        % (
            opt.dataroot,
            opt.roi_dir,
            opt.ignore_missing_moltype,
