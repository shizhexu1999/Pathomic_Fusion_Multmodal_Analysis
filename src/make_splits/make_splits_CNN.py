"""!@file make_splits_CNN.py
@brief This file is used to create the 15 fold cross validation splits for testing CNN
@details The original code that tests CNN is incorrect because it extracts the image features twice,
we have corrected this issue in our data splits.
@author Shizhe Xu
@date 28 June 2024
"""
...
def getAlignedMultimodalData(opt, model, device, all_dataset, pat_split, id2img):
    ...
    for pat_name in pat_split:
        ...
        for img_fname in id2img[pat_name]:
            ...
            features, hazard = get_vgg_features(
                    model,
                    device,
                    os.path.join(opt.dataroot, opt.roi_dir, img_fname),
                )
            x_patname.append(pat_name)
            x_path.append(features)
            x_hazard.append(hazard)
            x_grph.append(
                os.path.join(
                    opt.dataroot,
    ...
    return x_patname, x_path, x_hazard, x_grph, x_omic, e, t, g
...
for k in pnas_splits.columns:
    ...
    test_x_patname, test_x_path, test_x_hazard, test_x_grph, test_x_omic, test_e, test_t, test_g = (
    ...
    train_data = {
        ...
        "x_hazard": np.array(train_x_hazard),
        "x_grph": train_x_grph,
        "x_omic": train_x_omic,
    ...
    test_data = {
        ...
        "x_hazard": np.array(test_x_hazard),
        "x_grph": test_x_grph,
        "x_omic": test_x_omic,
...
pickle.dump(
    data_dict,
    open(
        "%s/splits_original/gbmlgg15cv_CNN_%s_%d_%d_%d%s.pkl"
        % (
            opt.dataroot,
            opt.roi_dir,
            opt.ignore_missing_moltype,
