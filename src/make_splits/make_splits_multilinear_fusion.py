"""!@file make_splits_multilinear_fusion.py
@brief This file is used to create the 15 fold cross validation splits that store CNN, CONCH, cell graph and genomic modalities
@author Shizhe Xu
@date 28 June 2024
"""
...
from networks_conch import define_net_conch
...
def get_conch_features(model, device, img_path):
    if model is None:
        return img_path
    else:
        x_path = Image.open(img_path).convert("RGB")
...
def getAlignedConchData(opt, model, device, all_dataset, pat_split, id2img):
    x_conch = []
    for pat_name in pat_split:
        if pat_name not in all_dataset.index:
            continue
...
for k in pnas_splits.columns:
    ...
    # if opt.make_all_train == 0 else pnas_splits.index
    pat_test = pnas_splits.index[pnas_splits[k] == "Test"]
    # the split is empty at the beginning
    cv_splits[int(k)] = {}

    # we leave the model as `None`
    # the get_vgg_features function will return the image path 
    model = None
    if opt.use_vgg_features:
        ...
        # it enables the parallel processing on multiple GPUs
        if isinstance(model, torch.nn.DataParallel):
            model = model.moduled
        model.load_state_dict(model_state_dict)
        model.eval()

    # obtain the data we expect to use for training and testing
    (
        train_x_patname,
        train_x_path,
        train_x_grph,
    ...
    model_conch = None
    if opt.use_conch_features:
        print("CONCH features are used in this case.")
        # load the trained CONCH model (pathology features)
        load_path = os.path.join(
            opt.checkpoints_dir,
            opt.exp_name,
            "conch",
        ...
        if hasattr(model_state_dict, "_metadata"):
            del model_state_dict._metadata
        # use the path model here (vgg)
        model_conch = define_net_conch(opt, None)
        # it enables the parallel processing on multiple GPUs
        if isinstance(model_conch, torch.nn.DataParallel):
            model_conch = model_conch.moduled
        model_conch.load_state_dict(model_state_dict)
        model_conch.eval()
    train_x_conch = getAlignedConchData(opt, model_conch, device, all_dataset, pat_train, id2img)
    test_x_conch = getAlignedConchData(opt, model_conch, device, all_dataset, pat_test, id2img)
    # reformat the data
    ...
    # using a standard scaler after removing
    # the extra dimension for the omic data
...
pickle.dump(
    data_dict,
    open(
        # "%s/splits_original/gbmlgg15cv_%s_%d_%d_%d%s_conch.pkl"
        "%s/splits/gbmlgg15cv_%s_%d_%d_%d%s_conch.pkl"
        % (
            opt.dataroot,
            opt.roi_dir,
            opt.ignore_missing_moltype,
