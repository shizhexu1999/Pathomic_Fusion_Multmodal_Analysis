"""!@file networks.py
@brief This file is used to define the network architecture for the model.
@details The file contains the network architecture for the model, including the PathNet, OmicNet, and GraphNet.
@author Shizhe Xu
@date 28 June 2024
"""
...
def define_net(opt, k):
    ...
    if opt.mode == "path":
        ...
        net = MultilinearNet(opt=opt, act=act, k=k)
...
def define_multilinearfusion(
    ...
    """
    fusion = None
    if fusion_type == "multilinearfusion":
        fusion = MultilinearFusion(
            skip=skip,
            use_bilinear=use_bilinear,
            gate1=gate1,
            gate2=gate2,
...
def get_vgg(
    ...
    **kwargs
    ...
    if pretrained:
        ...
        # list parameters in the pretrained model
        for key in list(pretrained_dict.keys()):
            # discard the classifier part of a pretrained model to reuse it
            # we only want the feature extractor part of the vgg19
...
class NormalizeFeaturesV2(object):
    ...
    def __call__(self, data):
        ...
        # for the first 12 manual features, including 
        # [short_axis, long_axis, angle, area, arc_length, eccentricity,
        # roundness, solidity], [dissimilarity, homogeneity, energy, ASM]
...
class GraphNet(torch.nn.Module):
    def __init__(
        ...
        # remove gnn=GNN from the original paper because the toch_geometric version has been updated
        # self.pool1 = torch_geometric.nn.SAGPooling(
        #     nhid, ratio=pooling_ratio, GNN=torch_geometric.nn.GCNConv
        # )
        ...
        # the original paper is using the graph neural network operator 
        # rather than the graph convolutional operator to calculate the projection scores
        self.pool1 = torch_geometric.nn.SAGPooling(
...
class MultilinearNet(nn.Module):
    def __init__(self, opt, act, k):
        ...
        if k is not None:
            ...
            self.fusion = define_multilinearfusion(
                fusion_type=opt.fusion_type,
                skip=opt.skip,
                use_bilinear=opt.use_bilinear,
                gate1=opt.path_gate,
    ...
    def forward(self, **kwargs):
        path_vec = kwargs["x_path"]
        conch_vec = kwargs["x_conch"]
        grph_vec, _ = self.grph_net(x_grph=kwargs["x_grph"])
        omic_vec, _ = self.omic_net(x_omic=kwargs["x_omic"])
        features = self.fusion(path_vec, grph_vec, omic_vec, conch_vec)
        hazard = self.classifier(features)
        if self.act is not None:
            hazard = self.act(hazard)
