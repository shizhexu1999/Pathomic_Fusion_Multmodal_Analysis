"""!@file train_test_multilinear_fusion.py
@brief This file contains the training and testing functions for the Multilinear/quadralinear fusion model.
@author Shizhe Xu
@date 28 June 2024
"""
...
from dataloader import PathgraphomicDatasetLoader, PathgraphomicFastDatasetLoader, MultilinearDatasetLoader
...
def train(opt, data, device, k):
    ...
    custom_data_loader = (
        MultilinearDatasetLoader(opt, data, split="train", mode=opt.mode)
        if opt.use_vgg_features and opt.use_conch_features
        else PathgraphomicFastDatasetLoader(opt, data, split="train", mode=opt.mode)
    ...
    for epoch in tqdm(range(opt.epoch_count, opt.niter + opt.niter_decay + 1)):
        ...
        for batch_idx, (x_path, x_grph, x_omic, x_conch, censor, survtime, grade) in enumerate(
            train_loader
        ):
...
def test(opt, model, data, split, device):
    ...
    custom_data_loader = (
        MultilinearDatasetLoader(opt, data, split="test", mode=opt.mode)
        if opt.use_vgg_features and opt.use_conch_features
        else PathgraphomicFastDatasetLoader(opt, data, split="test", mode=opt.mode)
        if opt.use_vgg_features
    ...
    for batch_idx, (x_path, x_grph, x_omic, x_conch, censor, survtime, grade) in enumerate(
        ...
        grade = grade.to(device) if "grad" in opt.task else grade
        _, pred = model(
            x_path=x_path.to(device), x_grph=x_grph.to(device), x_omic=x_omic.to(device), x_conch=x_conch.to(device),
        )
